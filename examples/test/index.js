...
const server = createServer((req, res) => {
  ...
  res.end(`Hello from /${process.env.TISSUE_NAME}\n`);
});

// 1. Graceful Shutdown: Finish existing requests before dying
process.on('SIGTERM', () => {
  console.log(`[tissue] /${process.env.TISSUE_NAME} received SIGTERM, closing gracefully...`);
...
server.listen(process.env.SOCKET_PATH, () => {
  ...
  console.log(`[tissue] /${process.env.TISSUE_NAME} ready`);
});